...
import (
	...
	"github.com/go-gost/x/internal/util/congestion"
...
func (d *quicDialer) initSession(ctx context.Context, addr net.Addr, conn net.PacketConn) (*quicSession, error) {
	...
	if d.md.tx > 0 {
		congestion.UseBrutal(session, uint64(d.md.tx*1024*1024/8), !d.md.txCompensation)
	} else {
		congestion.UseBBR(session, d.md.bbrLevel)
	}
