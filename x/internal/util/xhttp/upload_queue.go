...
// upload_queue is a specialized priority-queue + channel that reorders
// generic packets by a sequence number. Ported from xray splithttp.
...
// Packet is either a buffered payload (packet-up) or a streaming reader
	...
	for len(h.heap) > 0 {
		...
		if packet.Seq > h.nextSeq {
			if len(h.heap) > h.maxPackets {
				// the reassembly buffer is too large and we want to constrain
				// memory usage somehow; tear down the connection and hope the
				// application retries.
...
// heap code taken from https://pkg.go.dev/container/heap
