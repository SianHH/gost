...
// PaddingMethod is the padding generation method.
...
// XPaddingPlacement describes where and how the padding is carried.
...
// XPaddingConfig is the padding generation instruction set.
...
// GetNormalizedXPaddingBytes returns the padding length range with the xray
// default of [100, 1000] bytes.
...
// GenerateTokenishPaddingBase62 produces a random base62 string whose huffman
// encoded length is close to targetHuffmanBytes.
...
// GeneratePadding returns the padding string for the given method and length.
...
// ExtractXPaddingFromRequest locates the padding in the request. When obfsMode
// is false it looks for the xray-default "x_padding" query key inside the
// Referer header (falling back to the request URL query). Otherwise it uses
// the configured key/header/placement.
...
// IsPaddingValid checks that the padding falls inside [from, to] bytes (repeat-x)
// or huffman-encoded bytes (tokenish), with a two-byte tolerance for tokenish.
