...
func (r *RingBuffer[T]) Offset(index int) *T {
	if r.Empty() || index >= r.Len() {
		panic("github.com/apernet/quic-go/internal/utils/ringbuffer: offset from invalid index")
	}
...
func (r *RingBuffer[T]) Front() *T {
	if r.Empty() {
		panic("github.com/apernet/quic-go/internal/utils/ringbuffer: front from an empty queue")
	}
...
func (r *RingBuffer[T]) Back() *T {
	if r.Empty() {
		panic("github.com/apernet/quic-go/internal/utils/ringbuffer: back from an empty queue")
	}
